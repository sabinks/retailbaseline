...
<div class="app-page-title">
    <div class="page-title-wrapper">
        <div class="page-title-heading">
            ...
            <div>
                <strong>Assign Admin(s) to Region</strong>
            </div>
        </div>
    </div>
</div>
<div class="main-card mb-3 card">
    <div class='card-header'>
       <strong> Assign Admins to {{$region->name}} </strong>
    </div>
    <div class="card-body">
        <form method="post" action="{{asset('assignRegion/'.$region->id)}}">
        <h5>Available Admins Name || select one or more</h5>
        @csrf
            <div class="form-group">
                <label for="sel2">Mutiple select list (hold shift to select more than one):</label>
                {{-- <input type="hidden" value="{{ $region->id }}" name="region"/> --}}
            ...
            <a  href="{{route('regions.index')}}" class="btn btn-sm btn-secondary">Cancel</a>
        </form>
    </div>
</div>
