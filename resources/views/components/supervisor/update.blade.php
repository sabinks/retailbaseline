...
<div class="main-card mb-3 card">
    <div class='card-header'>
        update Supervisors <br>
        @if (session('message'))
          <p class="text-success">{{session('message')}}</p>
        @endif
    </div>
    <div class="card-body">
        <form method="post" action="{{route('supervisors.update',$user->id)}}" enctype="multipart/form-data" onsubmit="return checkForm(this);">
        @csrf
        @method('put')
            <div class="row">
            ...
            <div class='row'>
                <div class='form-group col-12 mb-0'>
                    ...
                    href="{{route('supervisors.index')}}"> Cancel<a></button>
                </div>
            </div>
        </form>
