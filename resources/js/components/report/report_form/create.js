...
let FormCreate = () => {
    ...
    useEffect(() => {
        axios.get('/report/create').then(res => {
            const { role, company, entities_list, creator_id, client_id } = res.data
            setState({
                ...state, role, company, entities_list, creator_id, client_id
            })
        ...
        <div>
            ...
            (state.toolbarItems && state.role != 'Field Staff' && state.role !== null) &&
                <>
                    ...
                    {renderErrorFor('data')}
                    <ReactFormBuilder toolbarItems={state.toolbarItems} onPost={onPost} />
                </>
            }
