...
const InfoReport = () => {
    ...
    useEffect(() => {
        axios.get('/report-data').then(res => {
    ...
    useEffect(() => {
        if (state.selected_entity) {
            if (state.selected_entity.length > 0) {
                ...
                axios.post('/all-staff-list', input).then(res => {
    ...
    const handleEntityGroupChange = (e, id) => {
        ...
        axios.get(`/get-entity-list/${value}`).then(res => {
    ...
    const handleCheckReport = async (reportdata_id, entitygroup_id) => {
        await axios.get(`/report-detail/${reportdata_id}/${entitygroup_id}`).then(res => {
    ...
    const assignReport = () => {
        const input = {
            ...
            report_id: selectReport.id,
            assign_date: state.assign_date,
            entitygroup_id: state.entitygroup_id,
            entity_ids: JSON.stringify(state.selected_entity.map(i => i.id))
        }

        axios.post('/assign-report-staff', input)
            .then(response => {
            ...
            <div className="main-card card">
                <div className='card-header'>
                    <div className='card-title'>
                        Assign Report Form To Field Staff
                    </div>
                    <div className="btn-wrapper btn-wrapper-multiple">

                        <Link className='btn btn-primary btn-sm float-right mb-2' to={'/report-generate'}>
                            Generate Report
                        </Link>
                        <Link className='btn btn-primary btn-sm float-right mb-2 mr-1' to={'/report-form/create'}>
                            Create Report
                        </Link>
                    </div>
                </div>
