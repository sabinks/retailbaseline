...
let EntityLocation = () => {
            ...
            if (role == 'Super Admin' || role == 'Admin') {
                ...
                if (company_staff) {
                    company_staff.map(region => {
                        ...
                        if(role == 'Admin'){
                            if (region.admin_staffs.length) {
                                let staffs = []
                                region.admin_staffs.map(staff => {
                                    staffs.push({
                                        value: staff.name, label: staff.name, id: staff.id
                                    })
                                })
    ...
    const getStaffLocation = (staff_id) => {
        axios.get(`/staff-current-location/${staff_id}`).then(res => {
            const { staff_location } = res.data

            const data = {
                lat: staff_location.lat,
                lng: staff_location.lng,
                name: staff_location.staff_detail.name,
                email: staff_location.staff_detail.email,
                phone: staff_location.staff_detail.phone_number

            }
            setStaffLocation(prev => ([
                data
            ]))
            displayMarkers()
        })
    ...
    const displayMarkers = () => {
        ...
        let location = value ? entityLocation : staffLocation
        if (location) {
            locations_markers.push(location.map((point, index) => {
                ...
                return <Marker key={index} id={index} position={{
                    ...
                    icon={value ?
                        point.assigned ?
                            "http://maps.google.com/mapfiles/ms/icons/red.png" : "http://maps.google.com/mapfiles/ms/icons/green.png"
    ...
    const onMarkerClick = (props, marker, e) => {
        ...
        let near_point = value == 1 ? entityLocation.filter(point => (Math.abs(Math.abs(parseFloat(point.lat)) - Math.abs(parseFloat(new_point.lat)))) <= 0.000009
            || Math.abs(Math.abs(parseFloat(point.lng)) - Math.abs(parseFloat(new_point.lng))) <= 0.000009) : staffLocation
        const displayData = {
            data: near_point
        }
        const temp = {
        ...
        <>
            <div className="row">
                ...
                <div className="col-md-6">
                    ...
                    {
                        ( role == 'Super Admin' || role == 'Admin' ) && value == 1 &&
                        <div className="float-right">
                    ...
                    {
                        ( role == 'Super Admin' || role == 'Admin' ) && value == 0 &&
            ...
            <div>
                {
                    ...
                    >
                        ...
                        <InfoWindow
                            ...
                            <div>
                                {
                                    value == 1 ? map.selectedPlace && (<h5>Entity List</h5>) : (
                                        <h5>Staff Detail</h5>)
                                }
